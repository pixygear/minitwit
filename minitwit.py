...
def before_request():
    ...
    if 'user_id' in session:
        ...
        g.users = query_db('select username as " " from user where user_id = ?', [session['user_id']], one = True)
...
def get_user_name(user_id):
    """Convenience method to look up the id for a username."""
    rv = g.db.execute('select username from user where userid = ?', session[user_id]).fetchone()
    return rv[0] if rv else None
